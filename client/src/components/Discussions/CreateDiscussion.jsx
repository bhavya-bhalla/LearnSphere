...
const CreateDiscussion = () => {
  ...
  const getAvailableCourses = () => {
    if (user?.role === 'student') {
      ...
      return mockCourses.filter(course => 
        user.enrolledCourses?.includes(course.id) || [1, 2].includes(course.id) // Default enrolled courses for demo
      );
    } else if (user?.role === 'instructor') {
      // Instructors can post in courses they teach
      return mockCourses.filter(course => course.instructorId === user.id);
    }
    return mockCourses; // Admin can post in any course
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            ...
            {availableCourses.length === 0 && (
              <p className="text-sm text-red-600 mt-1">
                You don't have access to any courses. Please contact your administrator.
              </p>
            )}
          </div>
      ...
      {/* Preview Section */}
      {discussionData.title && discussionData.content && (
        <div className="bg-white rounded-xl p-6 shadow-sm border border-secondary-200">
